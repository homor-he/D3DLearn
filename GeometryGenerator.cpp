...
void GeometryGenerator::CreateSolidSphere(float radius, SolidMeshData & meshData, UINT sliceCount, UINT stackCount)
{
	meshData.Vertices.clear();
	meshData.Indices.clear();
...
void GeometryGenerator::CreateGrid(float width, float depth, UINT m, UINT n, MeshData& meshData)
{
	...
	for(UINT i = 0; i < m; ++i)
	{
		...
		for(UINT j = 0; j < n; ++j)
		{
			...
			meshData.Vertices[i*n+j].TexC.x = j*du;       //在网格上拉伸纹理
